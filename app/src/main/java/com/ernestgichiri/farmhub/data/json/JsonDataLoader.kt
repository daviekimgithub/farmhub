...
class JsonDataLoader() {
    ...
    fun loadProductEntities(): Flow<NetworkResponseState<List<ProductEntity>>> = flow {
        ...
        try {
            ...
            Log.e("exception getting the list", e.toString())
            emit(NetworkResponseState.Error(e)) // Emit error state
        }
    }
    fun loadProductEntityById(productId: Int): Flow<NetworkResponseState<ProductEntity>> = flow {
        emit(NetworkResponseState.Loading) // Emit loading state
        try {
            ...
            if (product != null) {
                emit(NetworkResponseState.Success(product)) // Emit success state
            } else {
                emit(NetworkResponseState.Error(Exception("Product not found"))) // Emit error if not found
            }
        } catch (e: Exception) {
            Log.e("exception getting product", e.toString())
            emit(NetworkResponseState.Error(e)) // Emit error state
        }
    }
}
